...
@patch("exporter.collectors.validator_collector.read_last_n_lines")
@patch("exporter.collectors.validator_collector.get_public_block_height", return_value=115)
@patch("exporter.collectors.validator_collector.config", autospec=True)
def test_validator_collector_metrics(mock_config, mock_public, mock_read, mock_env_validator, dummy_log):
...
@patch("exporter.collectors.validator_collector.get_public_block_height", return_value=0)
@patch("exporter.collectors.validator_collector.config", autospec=True)
